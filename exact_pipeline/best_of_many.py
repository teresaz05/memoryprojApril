...
def build_cluster_memory_text(bank_units: Sequence[Dict[str, Any]], style: str) -> str:
    # The answering agent expects the same cluster-bank text layout as the existing
    # prose pipeline, so we rebuild the memory blob through the same helper.
...
def parse_merge_groups_allow_overlap(raw: str, valid_bank_ids: Sequence[str]) -> List[List[str]]:
    ...
    # Every bank still needs a path through the merge rounds, so banks omitted by
    # the planner are carried forward as singleton groups.
...
def select_scored_bank_units(
    ...
    if not bank_units:
        return []
    bank_unit_by_id = {
        str(bank_unit.get("bank_id", "") or "").strip(): bank_unit
        for bank_unit in bank_units
        if str(bank_unit.get("bank_id", "") or "").strip()
    }
    selected: List[Dict[str, Any]] = []
    for row in list(scored_rows)[: max(0, int(keep_count))]:
        bank_id = str(row.get("bank_id", "") or "").strip()
        bank_unit = bank_unit_by_id.get(bank_id)
        if bank_unit is not None:
            selected.append(bank_unit)
    return selected or list(bank_units)
...
def run_prose_oracle_bestofmany_experiment(
    *,
    ...
    if skip_merge:
        selected_final_bank_units = select_scored_bank_units(
            initial_bank_units,
            best_layer1_record["score_rows"],
            best_layer1_record["keep_count"],
        )
        best_merge_record = {
            "attempt_index": 0,
            "bank_count": len(initial_bank_units),
            "keep_count": int(best_layer1_record["keep_count"]),
            "top_score": float(best_layer1_record["top_score"]),
            "mean_top_score": float(best_layer1_record["mean_top_score"]),
            "sum_top_score": float(best_layer1_record["sum_top_score"]),
            "score_rows": list(best_layer1_record["score_rows"]),
            "score_map": dict(best_layer1_record["score_map"]),
            "final_bank_units": initial_bank_units,
            "selected_final_bank_units": selected_final_bank_units,
            "merge_round_summaries": [],
            "final_merged_cluster_banks": selected_doc_cluster_banks,
            "merge_rounds_completed": 0,
            "stop_reason": "merge_skipped",
        }
        ...
        persist_checkpoint("merge_skipped")
        if trace_handle is not None:
            write_jsonl_row(
                trace_handle,
                {
        ...
        for attempt_index in range(len(merge_attempt_records) + 1, max_merge_attempts + 1):
            ...
            selected_final_bank_units = select_scored_bank_units(
                final_bank_units,
                score_info["scored_rows"],
                score_info["keep_count"],
            )
            attempt_record = {
                "attempt_index": attempt_index,
    ...
    selected_final_bank_units = list(best_merge_record.get("selected_final_bank_units") or final_bank_units)
    ...
    if isinstance(selection_state, dict):
        ...
        memory_text = build_cluster_memory_text(selected_final_bank_units, doc_cluster_style)
    ...
    out_row = {
        ...
        "selected_final_banks": serialize_bank_units(selected_final_bank_units),
