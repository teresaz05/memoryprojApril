...
class SyntheticFilesystemState:
    ...
    def create_cluster(
        ...
        *,
        ...
        if not cluster_text.strip():
            raise ValueError("cluster_text must be non-empty")
    ...
    def merge_clusters(
        ...
        *,
        ...
        title = self._validate_title(title)
        if not merge_summary_text.strip():
            raise ValueError("merge_summary_text must be non-empty")
        if not child_paths:
        ...
        merge_rec = SyntheticFileRecord(
            ...
            title=title,
            slug=self._slugify(title),
            content=merge_summary_text.strip(),
            created_round=self.operation_round,
            parent_id=self._resolve_parent_id(parent_path),
...
class SyntheticFilesystemReward:
    ...
    async def __call__(self, history: list[Message]) -> tuple[float, dict[str, float]]:
        ...
        answerer_output = answerer_result["answer_text"]
        ...
        reward = correct if terminal_step >= 1.0 else 0.0
        if self.step_penalty > 0.0:
            reward -= self.step_penalty * float(assistant_turns)
        ...
        if terminal_step >= 1.0:
            reward -= self.raw_usage_ratio_penalty * raw_usage_ratio
            reward += self.synthetic_usage_bonus * synthetic_usage_ratio
...
class RedactingAgentToolMessageEnv(MessageEnv):
    ...
    compaction_model: str = "gemini-2.5-flash-lite"
