...
_MODAL_QWEN_DIAGNOSTIC_COUNT = 0


def apply_modal_qwen_non_thinking_defaults(payload: dict[str, Any], base_url: str) -> None:
    """Apply Modal/Qwen chat-template overrides, with an OpenRouter-compat escape hatch."""
    if "modal.run" not in base_url:
        return
    thinking_mode = os.getenv("MODAL_QWEN_THINKING_MODE", "disabled").lower()
    if thinking_mode in {"disabled", "false", "off", "0"}:
        ...
        raise ValueError(
            "MODAL_QWEN_THINKING_MODE must be one of disabled, enabled, or omit"
        )
    use_recommended = os.getenv("MODAL_QWEN_USE_RECOMMENDED_DEFAULTS", "").lower() in {
        "1",
        "true",
        "yes",
    }
    if not use_recommended:
        return
    payload["temperature"] = float(os.getenv("MODAL_QWEN_TEMPERATURE", "0.7"))
    payload["top_p"] = float(os.getenv("MODAL_QWEN_TOP_P", "0.8"))
    payload["top_k"] = int(os.getenv("MODAL_QWEN_TOP_K", "20"))
    payload["min_p"] = float(os.getenv("MODAL_QWEN_MIN_P", "0.0"))
    payload["presence_penalty"] = float(os.getenv("MODAL_QWEN_PRESENCE_PENALTY", "1.5"))
    payload["repetition_penalty"] = float(os.getenv("MODAL_QWEN_REPETITION_PENALTY", "1.0"))
...
def _is_modal_qwen_url(url: str) -> bool:
    return "modal.run" in url
...
def _preview_text(text: str, limit: int | None = None) -> dict[str, str | int | bool]:
    preview_limit = limit or int(os.getenv("MODAL_QWEN_DIAGNOSTIC_PREVIEW_CHARS", "200"))
    return {
        "chars": len(text),
...
def _modal_qwen_diagnostics_enabled(url: str) -> bool:
    return _is_modal_qwen_url(url) and _env_flag("MODAL_QWEN_DIAGNOSTICS", False)


def _should_log_modal_qwen_diagnostic(url: str) -> bool:
    global _MODAL_QWEN_DIAGNOSTIC_COUNT
    if not _modal_qwen_diagnostics_enabled(url):
        return False
    limit = int(os.getenv("MODAL_QWEN_DIAGNOSTIC_LIMIT", "200"))
    every = int(os.getenv("MODAL_QWEN_DIAGNOSTIC_EVERY", "0"))
    _MODAL_QWEN_DIAGNOSTIC_COUNT += 1
    count = _MODAL_QWEN_DIAGNOSTIC_COUNT
...
def _strip_unused_modal_qwen_response_fields(url: str, body: dict[str, Any]) -> dict[str, Any]:
    if not _is_modal_qwen_url(url):
        return body
    if _env_flag("MODAL_QWEN_KEEP_RESPONSE_TOKEN_DETAILS", False):
        return body
...
def _log_modal_qwen_response_diagnostics(
    *,
    url: str,
    ...
    if not _should_log_modal_qwen_diagnostic(url):
        return
    ...
    LOGGER.info(
        "modal_qwen_response_diagnostics %s",
        json.dumps(diagnostic, ensure_ascii=False, sort_keys=True),
    )


def _log_modal_qwen_http_error(
    *,
    ...
    LOGGER.info(
        "modal_qwen_http_error_diagnostics %s",
        json.dumps(diagnostic, ensure_ascii=False, sort_keys=True),
    )
...
def call_chat_model_messages(
    *,
    ...
    if backend == "openrouter":
        ...
        apply_modal_qwen_non_thinking_defaults(payload, base_url)
        ...
        choices = body.get("choices") or []
        if not choices:
            _log_modal_qwen_response_diagnostics(
                url=url,
                payload=payload,
                body=body,
                content_text="",
    ...
    for attempt in range(1, MODEL_HTTP_MAX_ATTEMPTS + 1):
        try:
            with urllib.request.urlopen(req, timeout=MODEL_HTTP_TIMEOUT_SECONDS) as resp:
                ...
                return _strip_unused_modal_qwen_response_fields(_request_url(req), body)
        except urllib.error.HTTPError as e:
            body = e.read().decode("utf-8", errors="replace")
            retrying = e.code in retryable_http_codes and attempt < MODEL_HTTP_MAX_ATTEMPTS
            _log_modal_qwen_http_error(
                req=req,
                status_code=e.code,
                body=body,
                attempt=attempt,
    ...
    def _source_blocks(
        ...
        for path in paths:
            ...
            try:
                ...
                # The state create path can tolerate mixed valid/invalid source
                # lists; the executor should not make those calls fail early.
                continue
