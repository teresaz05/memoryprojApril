...
AnswererWorkspaceMode = Literal["synthetic_plus_raw", "synthetic_only"]
...
_QWEN_HTTP_DIAGNOSTIC_COUNT = 0


def apply_qwen_http_defaults(payload: dict[str, Any], base_url: str) -> None:
    if not _env_flag("QWEN_HTTP_COMPAT", False):
        return
    thinking_mode = os.getenv("QWEN_HTTP_THINKING_MODE", "disabled").lower()
    if thinking_mode in {"disabled", "false", "off", "0"}:
        payload["chat_template_kwargs"] = {"enable_thinking": False}
    elif thinking_mode in {"enabled", "true", "on", "1"}:
        payload["chat_template_kwargs"] = {"enable_thinking": True}
    elif thinking_mode in {"omit", "openrouter", "default"}:
        pass
    else:
        raise ValueError(
            "QWEN_HTTP_THINKING_MODE must be one of disabled, enabled, or omit"
        )
    use_recommended = os.getenv("QWEN_HTTP_USE_RECOMMENDED_DEFAULTS", "").lower() in {
        "1",
        "true",
        "yes",
    }
    if not use_recommended:
        return
    payload["temperature"] = float(os.getenv("QWEN_HTTP_TEMPERATURE", "0.7"))
    payload["top_p"] = float(os.getenv("QWEN_HTTP_TOP_P", "0.8"))
    payload["top_k"] = int(os.getenv("QWEN_HTTP_TOP_K", "20"))
    payload["min_p"] = float(os.getenv("QWEN_HTTP_MIN_P", "0.0"))
    payload["presence_penalty"] = float(os.getenv("QWEN_HTTP_PRESENCE_PENALTY", "1.5"))
    payload["repetition_penalty"] = float(os.getenv("QWEN_HTTP_REPETITION_PENALTY", "1.0"))
...
def _is_qwen_http_url(url: str) -> bool:
    return _env_flag("QWEN_HTTP_COMPAT", False)


def _request_url(req: urllib.request.Request) -> str:
    try:
...
def _preview_text(text: str, limit: int | None = None) -> dict[str, str | int | bool]:
    preview_limit = limit or int(os.getenv("QWEN_HTTP_DIAGNOSTIC_PREVIEW_CHARS", "200"))
    return {
        "chars": len(text),
        "contains_think": "<think" in text.lower() or "</think>" in text.lower(),
        "prefix": text[:preview_limit],
        "suffix": text[-preview_limit:] if len(text) > preview_limit else text,
        "truncated": len(text) > preview_limit,
    }


def _qwen_http_diagnostics_enabled(url: str) -> bool:
    return _is_qwen_http_url(url) and _env_flag("QWEN_HTTP_DIAGNOSTICS", False)


def _should_log_qwen_http_diagnostic(url: str) -> bool:
    global _QWEN_HTTP_DIAGNOSTIC_COUNT
    if not _qwen_http_diagnostics_enabled(url):
        return False
    limit = int(os.getenv("QWEN_HTTP_DIAGNOSTIC_LIMIT", "200"))
    every = int(os.getenv("QWEN_HTTP_DIAGNOSTIC_EVERY", "0"))
    _QWEN_HTTP_DIAGNOSTIC_COUNT += 1
    count = _QWEN_HTTP_DIAGNOSTIC_COUNT
    return count <= limit or (every > 0 and count % every == 0)
...
def _strip_unused_qwen_http_response_fields(url: str, body: dict[str, Any]) -> dict[str, Any]:
    if not _is_qwen_http_url(url):
        return body
    if _env_flag("QWEN_HTTP_KEEP_RESPONSE_TOKEN_DETAILS", False):
        return body
    body.pop("prompt_token_ids", None)
    body.pop("prompt_logprobs", None)
...
def _log_qwen_http_response_diagnostics(
    *,
    url: str,
    ...
    if not _should_log_qwen_http_diagnostic(url):
        return
    choices = body.get("choices") or []
    choice = choices[0] if choices and isinstance(choices[0], dict) else {}
    message = choice.get("message") if isinstance(choice.get("message"), dict) else {}
    messages = payload.get("messages") or []
    payload_message_chars = sum(
    ...
    diagnostic = {
        ...
        "payload_message_chars": payload_message_chars,
        ...
        "content": _preview_text(content_text),
        "message_keys": sorted(message.keys()) if isinstance(message, dict) else [],
        "body_keys": sorted(body.keys()),
    }
    LOGGER.info(
        "qwen_http_response_diagnostics %s",
        json.dumps(diagnostic, ensure_ascii=False, sort_keys=True),
    )


def _log_qwen_http_error(
    *,
    ...
    diagnostic = {
        ...
        "attempt": attempt,
        "retrying": retrying,
        "error_body": _preview_text(body),
    }
    LOGGER.info(
        "qwen_http_error_diagnostics %s",
        json.dumps(diagnostic, ensure_ascii=False, sort_keys=True),
    )
...
def build_answerer_system_prompt(mode: AnswererWorkspaceMode) -> str:
    ...
    if mode == "synthetic_plus_raw":
        workspace_lines.append("- raw_docs/ as a strict fallback exposed only through synthetic-file provenance")
    ...
    if mode == "synthetic_plus_raw":
        order_lines.append("3. use raw_docs/ only if the synthetic files still lack a decisive detail")
        guidance = (
            "Do not guess. If the synthetic files you have read so far do not contain enough evidence to "
            "uniquely identify the answer, continue exploring. If the synthetic filesystem is still "
            "insufficient, follow provenance to raw_docs/ as a fallback before answering."
        )
    else:
        guidance = (
            "Do not guess. If the synthetic files you have read so far do not contain enough evidence to "
...
def build_answerer_user_prompt(question: str, mode: AnswererWorkspaceMode) -> str:
    if mode == "synthetic_plus_raw":
        return (
            ...
            "Answer from the synthetic filesystem if possible; use raw_docs/ only as provenance-linked fallback."
        )
    return (
        f"Question: {question}\n\n"
        "Use the workspace tools to inspect the final workspace. "
...
def call_chat_model_messages(
    *,
    ...
    if backend == "openrouter":
        ...
        apply_qwen_http_defaults(payload, base_url)
        if max_output_tokens is not None and max_output_tokens > 0:
            payload["max_tokens"] = max_output_tokens
        if response_json:
            system_texts.insert(0, "Return only one compact JSON object.")
        if system_texts:
        ...
        choices = body.get("choices") or []
        if not choices:
            _log_qwen_http_response_diagnostics(
                url=url,
                payload=payload,
                body=body,
                content_text="",
        ...
        content_text = _openai_content_to_text(content)
        _log_qwen_http_response_diagnostics(
            url=url,
            payload=payload,
            body=body,
    ...
    for attempt in range(1, MODEL_HTTP_MAX_ATTEMPTS + 1):
        try:
            with urllib.request.urlopen(req, timeout=MODEL_HTTP_TIMEOUT_SECONDS) as resp:
                ...
                return _strip_unused_qwen_http_response_fields(_request_url(req), body)
        except urllib.error.HTTPError as e:
            body = e.read().decode("utf-8", errors="replace")
            retrying = e.code in retryable_http_codes and attempt < MODEL_HTTP_MAX_ATTEMPTS
            _log_qwen_http_error(
                req=req,
                status_code=e.code,
                body=body,
                attempt=attempt,
...
class SyntheticFilesystemDatum(TypedDict):
    ...
    amortization_questions: list[dict[str, str]]
...
class SyntheticFilesystemState:
    ...
    def generate_readme(self, *, include_raw_docs: bool = False) -> str:
        lines = [
        ...
        if include_raw_docs:
            lines.append("4. If the synthetic filesystem still lacks a decisive detail, use raw_docs/ only through provenance links exposed by synthetic files.")
        lines.extend(
            [
            "",
            "Conventions:",
            ...
            "- Raw documents are fallback evidence, not primary context.",
    ...
    def materialize_final_workspace(
        ...
        include_raw_docs = mode == "synthetic_plus_raw"
        (root_dir / "README.md").write_text(
            self.generate_readme(include_raw_docs=include_raw_docs), encoding="utf-8"
        )

        if include_raw_docs:
            raw_docs_dir = root_dir / "raw_docs"
            raw_docs_dir.mkdir(parents=True, exist_ok=True)
            for rel in self.visible_raw_doc_paths:
                source = self.resolve_raw_doc(f"raw_docs/{rel}")
                target = raw_docs_dir / rel
...
class FrozenSyntheticFileExecutor:
    ...
    def _clean_generated_text(self, text: str, *, title: str, source_blocks: str, kind: str) -> str:
        cleaned = str(text or "").strip()
        ...
        for marker in (
            ...
            if marker.lower() in cleaned.lower():
                ...
                break
        cleaned = re.sub(
            r"(?i)^(cluster text|merge summary text|final output|final answer|final|output)\s*:\s*",
            "",
            cleaned,
        ).strip()
...
class SyntheticFilesystemReward:
    ...
    gold_answer: str
    question: str
    amortization_questions: list[dict[str, str]] = field(default_factory=list)
    ...
    last_workspace_docs: list[dict[str, str]] = field(default_factory=list, init=False)
    ...
    async def _run_amortized_scored_answerers(
        self,
        *,
        repeats: int,
    ) -> tuple[dict[str, Any], dict[str, float], dict[str, float], int]:
        questions = [
            q
            for q in self.amortization_questions
            if str(q.get("question", "")).strip()
            and str(q.get("gold_answer", "")).strip()
        ]
        if not questions:
            return await self._run_scored_answerer_repeats(repeats=repeats)

        old_question = self.question
        old_gold_answer = self.gold_answer
        scored: list[tuple[dict[str, Any], dict[str, float], dict[str, float], int]] = []
        try:
            for question_row in questions:
                self.question = str(question_row["question"])
                self.gold_answer = str(question_row["gold_answer"])
                scored.append(await self._run_scored_answerer_repeats(repeats=repeats))
                gc.collect()
        finally:
            self.question = old_question
            self.gold_answer = old_gold_answer
        ...
        primary_result = dict(
            next(
                (
                    result
                    for result, _, metrics, _ in scored
                    if float(metrics.get("answerer_error", 0.0)) <= 0.0
                ),
                scored[0][0],
            )
        ...
        primary_result["steps"] = mean_float(
            [float(result.get("steps", 0.0) or 0.0) for result, _, _, _ in scored]
        )
        for key in [
            "used_raw_docs",
            "used_readme",
            ...
            primary_result[key] = mean_float(
                [float(bool(result.get(key, False))) for result, _, _, _ in scored]
            )

        usage_keys = sorted(set().union(*(usage.keys() for _, usage, _, _ in scored)))
        avg_usage = {
            key: mean_float([usage.get(key, 0.0) for _, usage, _, _ in scored])
            for key in usage_keys
        }
        score_keys = sorted(set().union(*(metrics.keys() for _, _, metrics, _ in scored)))
        avg_score_metrics = {
            key: mean_float([metrics.get(key, 0.0) for _, _, metrics, _ in scored])
            for key in score_keys
        }
        avg_score_metrics["amortization_num_questions"] = float(len(questions))
        avg_score_metrics["amortization_correct_sum"] = sum(
            float(metrics.get("correct", 0.0)) for _, _, metrics, _ in scored
        )
        avg_score_metrics["amortization_exact_sum"] = sum(
            float(metrics.get("exact_match", 0.0)) for _, _, metrics, _ in scored
        )
        total_repeats = sum(int(repeat_count) for _, _, _, repeat_count in scored)
        return primary_result, avg_usage, avg_score_metrics, total_repeats
    ...
    async def __call__(self, history: list[Message]) -> tuple[float, dict[str, float]]:
        ...
        if terminal_step >= 1.0:
            if not (self.answerer_workspace_mode == "synthetic_only" and not active_records):
                answerer_result, usage, score_metrics, answerer_repeats_used = (
                    await self._run_amortized_scored_answerers(
        ...
        if "amortization_num_questions" in score_metrics:
            metrics["amortization_num_questions"] = score_metrics["amortization_num_questions"]
            metrics["amortization_correct_sum"] = score_metrics.get("amortization_correct_sum", 0.0)
            metrics["amortization_exact_sum"] = score_metrics.get("amortization_exact_sum", 0.0)
            metrics["amortization_correct_rate"] = correct
            metrics["amortization_exact_rate"] = exact_match
        if terminal_step >= 1.0 and answerer_evaluated >= 1.0:
            answerer_read_paths = [
                str(path) for path in answerer_result.get("read_paths", [])
            ]
            self.last_answerer_trace = {
                "answer_text": str(answerer_result.get("answer_text", ""))[:4000],
                "read_paths": answerer_read_paths,
                "synthetic_read_paths": [
                    path
                    for path in answerer_read_paths
                    if path != "README.md" and not path.startswith("raw_docs/")
                ],
                "raw_doc_read_paths": [
                    path for path in answerer_read_paths if path.startswith("raw_docs/")
                ],
                ...
                "amortization_num_questions": int(score_metrics.get("amortization_num_questions", 1.0)),
            }
        metrics.update(
            {
    ...
    async def _run_answerer(self) -> dict[str, Any]:
        with tempfile.TemporaryDirectory(prefix="synthetic_fs_answerer_") as tmpdir:
            ...
            self.last_workspace_docs = _collect_answer_facing_workspace_docs(workspace_root)
            tools = ReadOnlyAnswererWorkspaceTools(workspace_root)
            root_bootstrap_entries = self.state.answerer_bootstrap_entries()
            workspace_path_to_node_id = self.state.workspace_path_to_node_id()
            provenance_memo: dict[str, set[str]] = {}
            allowed_raw_docs: set[str] = set()
            for rec in self.state.root_active_records():
                allowed_raw_docs.update(
                    self.state._transitive_raw_docs_for_node(rec.node_id, provenance_memo)
                )
            ...
            def list_files_payload(path: str) -> dict[str, Any]:
                clean = path.strip() or "."
                if self.answerer_workspace_mode == "synthetic_plus_raw" and (
                    clean == "raw_docs" or clean.startswith("raw_docs/")
                ):
                    return self.state._virtual_list_from_paths(clean, sorted(allowed_raw_docs))
            ...
            for step in range(1, self.answerer_max_turns + 1):
                ...
                try:
                    if action["action"] == "list_files":
                        ...
                        if (
                            self.answerer_workspace_mode == "synthetic_plus_raw"
                            and path.startswith("raw_docs/")
                            and path not in allowed_raw_docs
                        ):
                            raise ValueError(
                                "Raw document is not available yet. Only use raw_docs/ through provenance from synthetic files already in context or read earlier."
                            )
                        ...
                        if path != "README.md" and not path.startswith("raw_docs/"):
                            ...
                            node_id = workspace_path_to_node_id.get(path)
                            if node_id is not None:
                                allowed_raw_docs.update(
                                    self.state._transitive_raw_docs_for_node(node_id, provenance_memo)
                                )
...
def _collect_answer_facing_workspace_docs(workspace_root: Path) -> list[dict[str, str]]:
    docs: list[dict[str, str]] = []
    for path in sorted(workspace_root.rglob("*")):
        if not path.is_file():
            continue
        rel = path.relative_to(workspace_root).as_posix()
        if rel == "README.md" or rel.startswith("raw_docs/"):
            continue
        docs.append(
            {
                "path": rel,
...
class RedactingAgentToolMessageEnv(MessageEnv):
    ...
    async def step(self, message: Message) -> MessageStepResult:
        ...
        if done:
            ...
            final_workspace_docs = getattr(self.reward_fn, "last_workspace_docs", None)
            if self.log_step_details and not final_workspace_docs and self.state is not None:
                with tempfile.TemporaryDirectory(prefix="synthetic_fs_export_") as tmpdir:
                    workspace_root = self.state.materialize_final_workspace(
                        Path(tmpdir) / "final_workspace",
                        mode=getattr(self.reward_fn, "answerer_workspace_mode", "synthetic_only"),
                    )
                    final_workspace_docs = _collect_answer_facing_workspace_docs(workspace_root)
            if self.log_step_details and final_workspace_docs:
                logs["final_workspace_docs"] = json.dumps(
                    final_workspace_docs, ensure_ascii=False
                )
...
def validate_builder_executor_config(enabled: bool, backend: str, base_url: str) -> None:
    if enabled and backend == "openrouter" and not base_url.strip():
        raise ValueError("builder_executor_base_url is required when builder_executor_enabled=True and builder_executor_backend='openrouter'.")
...
class SyntheticFilesystemEnvGroupBuilder(EnvGroupBuilder):
    ...
    async def make_envs(self) -> Sequence[Env]:
        validate_builder_executor_config(
            self.builder_executor_enabled,
            self.builder_executor_backend,
            self.builder_executor_base_url,
        )
        ...
        for _ in range(self.group_size):
            ...
            reward_fn = SyntheticFilesystemReward(
                ...
                amortization_questions=list(self.datum.get("amortization_questions", [])),
...
def hydrate_privileged_record(row: dict[str, Any]) -> SyntheticFilesystemDatum:
    ...
    return {
        ...
        "amortization_questions": list(row.get("amortization_questions", [])),
    }
