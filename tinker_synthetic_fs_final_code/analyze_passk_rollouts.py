    ...
    for source_idx, path in enumerate(_summary_paths(args.run_dirs)):
        with path.open("r", encoding="utf-8") as handle:
            for line in handle:
                ...
                grouped[group_idx].append(
                    {
                        "source_idx": source_idx,
                        "path": str(path),
                        "traj_idx": int(row.get("traj_idx", len(grouped[group_idx]))),
                        "metrics": metrics,
                    }
                )

    per_question: list[dict[str, Any]] = []
    for ordinal, group_idx in enumerate(sorted(order)):
        trajectories = sorted(grouped[group_idx], key=lambda item: (item["source_idx"], item["traj_idx"]))[: args.k]
        if not trajectories:
            continue
