...
BASE_EXPERIMENT_NAME = 'prose_oracle_bestofmany'
METHOD_NAME = 'oracle_doc_cluster_bank_prose_oracle_bestofmany_concat_titled'
VARIANT_NAME = 'oracle_doc_cluster_bank_prose_oracle_bestofmany_concat_titled'


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run best-of-many experiment on the packaged q50 support-only dataset.')
    ...
    parser.add_argument('--max-layer1-attempts', type=int, default=5)
    parser.add_argument('--min-layer1-attempts', type=int, default=1)
    parser.add_argument('--single-layer1-attempt', action='store_true', help='Run exactly one layer-1 attempt while still allowing repeated merge attempts.')
    parser.add_argument('--max-merge-attempts', type=int, default=5)
    parser.add_argument('--min-merge-attempts', type=int, default=1)
    parser.add_argument('--max-merge-rounds', type=int, default=5)
    parser.add_argument('--score-fraction', type=float, default=0.20)
...
def experiment_name(single_layer1_attempt: bool) -> str:
    return BASE_EXPERIMENT_NAME + ('_single_layer1' if single_layer1_attempt else '')


def build_default_run_dir(answer_model: str, single_layer1_attempt: bool) -> Path:
    return PACKAGE_ROOT / 'runs' / 'support_only_q50' / model_slug(answer_model) / experiment_name(single_layer1_attempt) / timestamp_now()
...
def main() -> None:
    ...
    run_dir = Path(args.run_dir) if args.run_dir else build_default_run_dir(answer_model, args.single_layer1_attempt)
    ...
    stem = 'q50_prose_oracle_bestofmany_single_layer1' if args.single_layer1_attempt else 'q50_prose_oracle_bestofmany'
    out_jsonl = run_dir / 'results' / f'{stem}.jsonl'
    trace_jsonl = run_dir / 'traces' / f'{stem}.trace.jsonl'
    manifest_json = run_dir / f'{stem}.manifest.json'
    graded_jsonl = run_dir / 'graded' / f'{stem}.scored.jsonl'
    ...
    with out_jsonl.open(mode, encoding='utf-8') as fout:
        ...
        try:
            print(
                f"[start] dataset={dataset_path} rows={len(rows)} max_layer1_attempts={args.max_layer1_attempts} max_merge_attempts={args.max_merge_attempts} max_merge_rounds={args.max_merge_rounds} score_fraction={args.score_fraction}",
                flush=True,
            )
            for row in rows:
                ...
                if qid in done_ids:
                    skipped_completed += 1
                    continue
                out_row = experiment.run_prose_oracle_bestofmany_experiment(
                    row=row,
                    ...
                    method=METHOD_NAME,
                    variant=VARIANT_NAME + ('_single_layer1' if args.single_layer1_attempt else ''),
                ...
                if args.progress_every > 0 and processed % args.progress_every == 0:
                    print(
                        f"[progress] processed={processed} last_qid={qid} layer1_attempt={out_row.get('selected_layer1_attempt_index')} merge_attempt={out_row.get('selected_merge_attempt_index')} merge_rounds_completed={out_row.get('merge_rounds_completed')}",
                        flush=True,
                    )
        finally:
            trace_handle.close()
