#!/usr/bin/env python3
"""Run the prose merge2 experiment without any cluster-specific wrapper code.

The underlying implementation is the same copied cluster-bank core used in the original
BrowseCompV2 runs. This wrapper only fills in the prose-specific summary mode, default paths,
and final grading passes.
"""
...
from april_version_code.methods import prose_merge2 as experiment


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run the prose merge2 experiment on the q50 support-only dataset.')
...
def main() -> None:
    ...
    # Call the copied cluster-bank core directly so the wrapper stays dependency-light.
    run_script_main(
        'cluster_bank_core',
    ...
    if not args.skip_grading:
        # Grade layer 1 and layer 2 outputs separately so they match the legacy run layout.
        run_script_main(
            'grading',
            grading_core.main,
            [
